...
for file in os.listdir('../In/Box_data/'): 
        ...
        if file.endswith("BLN.xlsx") or file.endswith("6WK.xlsx"):
            ...
            file_n = root + file[0:7] + '_sen_acc.csv'          
            sensor_acc = pd.read_excel(root + file, sheet_name='Sensor Free Acceleration') # , skiprows=[0], usecols="B:BR"
            sensor_acc.to_csv(file_n, header=True, index=False) #   
         

            file_n = root + file[0:7] + '_sen_eul.csv'          
            sensor_eul = pd.read_excel(root + file, sheet_name='Sensor Orientation - Euler') # , skiprows=[0], usecols="B:BR"
            sensor_eul.to_csv(file_n, header=True, index=False) #    

            file_n = root + file[0:7] + '_sen_qua.csv'          
            sensor_qua = pd.read_excel(root + file, sheet_name='Sensor Orientation - Quat') # , skiprows=[0], usecols="B:BR"
            sensor_qua.to_csv(file_n, header=True, index=False) #          
