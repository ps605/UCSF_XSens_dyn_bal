...
for i_csv in range(len(csv_files)):
    ...
    data_q0123 = pd.read_csv(trial_name[:-3] + 'sen_qua.csv')  
   ...
   # Initialise unwrap array
    ...
    eul_recon_uwr = np.rad2deg(eul_recon_uwr)

    # Check Euler with plots
    lim = 270 # 3/2*np.pi
    fig, (ax1, ax2, ax3) = plt.subplots(1,3)
    plt.ioff()
    fig.suptitle(csv_file[:-4])
    # ax1.plot(eul_x_T8[:1200], label="IMU")    
    # ax1.plot(np.rad2deg(roll[:1200]),label="RoPiYa")
    ax1.plot(eul_recon[:1200,2],label="ZYX")
    # ax1.plot(eul_recon_filt[:1200,2], label="ZYXfilt")
    ax1.plot(eul_recon_uwr[:1200,2], label="ZYXfiltUrp")
    ax1.set_ylim([-lim,lim])
    ax1.set_title('X')

    # ax2.plot(eul_y_T8[:1200])
    # ax2.plot(np.rad2deg(pitch[:1200]))
    ax2.plot(eul_recon[:1200,1])
    # ax2.plot(eul_recon_filt[:1200,1])
    ax2.plot(eul_recon_uwr[:1200,1])
    ax2.set_ylim([-lim,lim])
    ax2.set_yticks([])
    ax2.set_title('Y')

    # ax3.plot(eul_z_T8[:1200])
    # ax3.plot(np.rad2deg(yaw[:1200]))
    ax3.plot(eul_recon[:1200,0])
    # ax3.plot(eul_recon_filt[:1200,0])
    ax3.plot(eul_recon_uwr[:1200,0])
    ...
    fig.legend()
    fig.savefig('../Out/Analysis/Paper/Figures/imu_signal_comp/recon_' + csv_file[:-4] + '.pdf')

    plt.figure()
    # plt.plot(eul_x_T8,eul_y_T8)
    plt.plot(eul_recon[:,2],eul_recon[:,1], label="ZYXfilt")
    plt.plot(eul_recon_uwr[:1200,2],eul_recon_uwr[:1200,1], label="ZYXfiltUrp")
    plt.xlim([-lim,lim])
    plt.ylim([-lim,lim])
    plt.xlabel('X rot')
    plt.xlabel('Y rot')
    plt.legend()
    plt.savefig('../Out/Analysis/Paper/Figures/imu_signal_comp/XvsY_' + csv_file[:-4] + '.pdf')
